...
- **Job portals:** "The framework includes tools for Naukri.com (India) and several Danish job portals (Jobindex, Jobbank, Jobdanmark, Jobnet). Are these the right ones for you, or do you use other sites (e.g. LinkedIn, Instahyre, Cutshort, Wellfound)?" Note: only Naukri has a live CLI integration for India — for other Indian/international sites without a built-in scraper (LinkedIn, Instahyre, Cutshort, Wellfound, etc.), the user can paste job posting text directly into `/apply` instead of relying on `/scrape`. If the user is outside India or Denmark entirely, acknowledge that the built-in CLI tools are specific to those two markets and suggest they can add their own portal integrations or rely on LinkedIn/Google site-searches plus pasted postings.
